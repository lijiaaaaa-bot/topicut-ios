...
## ADR-0022 工作台内编辑 EDL（不重跑切片）

- 状态: 已采纳，已实现（2026-09-12）
- 决策: 成片页点选中行打开底部编辑页，而不是只读依据。用户用大号左右手柄改当前切片的起止、在合法时与下一条合并、或丢弃当前条；点「完成」把同一份 `schema_version` 1 的 EDL 写回 `ProjectStore`。不重跑转写，也不再调用 DeepSeek。被改过的切片删除其 `<clipID>.mp4` 并回到未导出，避免旧文件冒充新范围。合并保留前一条的 id / 标题，后一条消失；两条之间的空隙记为 removed。界面只有动作和真实时间码，没有建议文案。
- 理由: 切片计划来自模型，发布前仍要改出入点和去掉多余条。重跑 ASR/LLM 会花时间和钱，且答案不稳定。EDL 本来就是渲染契约，在契约上改比再问模型一次更直接。字段未变，所以不升 `schema_version`。
- 后果: `EDLEdit` / `EDLEditor` 做纯校验重写；`SliceSession.applyEditedDocument` 落盘并清导出。ADR-0010 补充里「点标题打开只读剪辑依据」被本条取代。列表左滑删除、长按合并、矩阵变体仍不在本条范围内。
