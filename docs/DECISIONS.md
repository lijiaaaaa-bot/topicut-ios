...
- 后果: 成片页的唯一等待是亚秒级的合成构建（用切片首帧垫底）。预览字幕是叠加层而非烧录（`animationTool` 仅导出可用），字号与导出略有差异，属可接受的已知偏差。预渲染取消后，切换切片/画幅不再触发任何导出，也不再占用 CPU/电量。`ClipRenderState` 只描述导出状态；列表行上的对勾表示「已导出」。iOS 模拟器不渲染带 `videoComposition` 的 `AVPlayerItem`（画面全黑、字幕叠加层正常），这是模拟器已知限制，真机正常；模拟器截图只用于核对布局与选中态。
